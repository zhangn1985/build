...
install_common()
{
	...
	if [[ x${USE_ARMBIAN_CONFIG} == x || ${USE_ARMBIAN_CONFIG} = "yes" ]]; then
		...
		cp -rp "${SRC}"/packages/bsp/common "${SRC}"/cache/
		rm -rf "${SRC}"/cache/common/etc/NetworkManager/
		rm -rf "${SRC}"/cache/common/etc/X11/
		rm -rf "${SRC}"/cache/common/etc/apt/apt.conf.d/81-armbian-no-languages
		rm -rf "${SRC}"/cache/common/etc/cron.d/
		rm -rf "${SRC}"/cache/common/etc/cron.daily/
		rm -rf "${SRC}"/cache/common/etc/default/armbian-ramlog.dpkg-dist
		rm -rf "${SRC}"/cache/common/etc/default/armbian-zram-config.dpkg-dist
		rm -rf "${SRC}"/cache/common/etc/initramfs/
		rm -rf "${SRC}"/cache/common/etc/kernel/preinst.d/
		rm -rf "${SRC}"/cache/common/etc/modprobe.d/
		rm -rf "${SRC}"/cache/common/etc/network/
		rm -rf "${SRC}"/cache/common/etc/profile.d/armbian-lang.sh
		rm -rf "${SRC}"/cache/common/etc/systemd/
		rm -rf "${SRC}"/cache/common/etc/udev/
		rm -rf "${SRC}"/cache/common/etc/update-motd.d/41-armbian-config
		rm -rf "${SRC}"/cache/common/lib/systemd/system/armbian-firstrun-config.service
		rm -rf "${SRC}"/cache/common/lib/systemd/system/armbian-firstrun.service
		rm -rf "${SRC}"/cache/common/lib/systemd/system/armbian-hardware-monitor.service
		rm -rf "${SRC}"/cache/common/lib/systemd/system/armbian-hardware-optimize.service
		rm -rf "${SRC}"/cache/common/lib/systemd/system/armbian-ramlog.service
		rm -rf "${SRC}"/cache/common/lib/systemd/system/armbian-zram-config.service
		rm -rf "${SRC}"/cache/common/lib/systemd/system/systemd-journald.service.d/
		rm -rf "${SRC}"/cache/common/usr/lib/armbian/armbian-firstrun
		rm -rf "${SRC}"/cache/common/usr/lib/armbian/armbian-firstrun-config
		rm -rf "${SRC}"/cache/common/usr/lib/armbian/armbian-hardware-monitor
		rm -rf "${SRC}"/cache/common/usr/lib/armbian/armbian-hardware-optimization
		rm -rf "${SRC}"/cache/common/usr/lib/armbian/armbian-ramlog
		rm -rf "${SRC}"/cache/common/usr/lib/armbian/armbian-truncate-logs
		rm -rf "${SRC}"/cache/common/usr/lib/armbian/armbian-zram-config
		rm -rf "${SRC}"/cache/common/usr/lib/chromium-browser/
		rm -rf "${SRC}"/cache/common/etc/initramfs/post-update.d/99-uboot
		cp -rp "${SRC}"/cache/common/* "${SDCARD}"/
		rm -rf "${SRC}"/cache/common/
