...
write_uboot()
{
	...
	mkdir -p /tmp/u-boot/
	dpkg -x "${DEB_STORAGE}/${CHOSEN_UBOOT}_${UBOOT_VERSION}+${REVISION}_${ARCH}.deb" /tmp/u-boot/
	write_uboot_platform "/tmp/u-boot/usr/lib/${CHOSEN_UBOOT}_${UBOOT_VERSION}+${REVISION}_${ARCH}" "$loop"
